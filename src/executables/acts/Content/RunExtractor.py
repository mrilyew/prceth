...
class CreateContentUnit(BaseAct):
    name = 'CreateContentUnit'
    category = 'Entities'
    docs = {
        "description": {
            "name": {
                "ru": "Создать запись",
                "en": "Create ContentUnit"
            },
            "definition": {
                "ru": "Создаёт запись из переданного экстрактора (параметр \"i\")",
                "en": "Creates new ContentUnit via provided extractor (\"i\" arg)"
            }
        },
        "returns": {
            "end": True,
    ...
    async def execute(self, args={}):
        ...
        collection_id = self.passed_params.get("collection_id", None) # Collection id to where ContentUnit will added
        ...
        if collection_id != None:
            ...
            if __coll == None:
                logger.log(section="ContentUnitSaveMechanism", name="error", message="Collection not found, not adding.")
            else:
        ...
        RETURN_ENTITIES = []
        for ContentUnit in EXTRACTOR_RESULTS.get("entities"):
            RETURN_ENTITIES.append(ContentUnit)
            for _ContentUnit in ContentUnit.getLinkedEntities():
                RETURN_ENTITIES.append(_ContentUnit)
        ...
        __RETURN_ENTITIES = []
        for RETURN_ContentUnit in RETURN_ENTITIES:
            if self.passed_params.get("return_raw") == True:
                __RETURN_ENTITIES.append(RETURN_ContentUnit)
            else:
                __RETURN_ENTITIES.append(RETURN_ContentUnit.getApiStructure())
